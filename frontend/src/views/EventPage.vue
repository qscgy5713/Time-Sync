<template>
  <div class="max-w-3xl mx-auto px-6 py-10">
    ...
    <div v-else>
      ...
      <div v-if="finalizedOption" class="mb-6 flex items-center justify-between gap-3 rounded-xl bg-green-50 border border-green-200 px-4 py-3">
        <div class="flex items-center gap-2 text-green-700">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" class="h-4.5 w-4.5 shrink-0">
        ...
        <button
          v-if="isOwner"
          type="button"
          class="shrink-0 text-xs font-medium text-green-700 underline decoration-dotted hover:text-green-900"
          :disabled="finalizing"
          @click="unfinalize"
        >
          取消鎖定
      ...
      <div v-else class="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-sm shadow-gray-100">
        ...
        <div v-if="isOwner && !finalizedOption" class="mb-5 rounded-xl border border-gray-100 bg-gray-50/60 p-4">
          <p class="text-xs font-semibold text-gray-600 mb-2">選一個時段鎖定成最終時間（鎖定後大家就不能再修改投票了）</p>
          <div class="flex flex-col sm:flex-row gap-2">
            <div class="flex-1">
              <CustomSelect v-model="finalizeSelection" :options="finalizeOptions" placeholder="選擇時段" />
            </div>
            <button
              type="button"
              class="shrink-0 px-5 py-2.5 rounded-lg bg-gradient-to-br from-brand-500 to-brand-600 text-white text-sm font-semibold shadow-sm shadow-brand-200 transition hover:shadow-md disabled:opacity-50 disabled:pointer-events-none"
              :disabled="finalizing || finalizeSelection == null"
              @click="finalizeSelected"
            >
              {{ finalizing ? '處理中…' : '鎖定此時段' }}
            </button>
          </div>
          <p v-if="finalizeError" class="mt-2 text-xs text-red-600">{{ finalizeError }}</p>
        </div>
...
import { getStoredParticipant, storeParticipant, clearStoredParticipant, getOwnerToken } from '../utils/identity'
...
const isOwner = computed(() => !!ownerToken.value)
const finalizeSelection = ref(null)
...
const finalizedOption = computed(() => {
  ...
  return { id: opt.id, label: local.toFormat('yyyy/LL/dd (ccc) HH:mm') }
})
...
async function finalizeSelected() {
  if (finalizeSelection.value == null) return
  finalizing.value = true
  finalizeError.value = ''
  try {
    await api.finalizeEvent(props.id, Number(finalizeSelection.value), ownerToken.value)
    await loadEvent()
  } catch (err) {
    finalizeError.value = err.message || '鎖定失敗，請稍後再試'
  } finally {
...
async function unfinalize() {
  ...
  finalizeError.value = ''
  try {
    await api.finalizeEvent(props.id, null, ownerToken.value)
    await loadEvent()
  } catch (err) {
    finalizeError.value = err.message || '取消鎖定失敗，請稍後再試'
  } finally {
    finalizing.value = false
  }
}
...
async function loadEvent() {
  ...
  loadError.value = ''
  try {
    ownerToken.value = getOwnerToken(props.id)
